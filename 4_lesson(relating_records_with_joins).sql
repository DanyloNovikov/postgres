...
-- Join joins the users table and since the comments table has links to the user table, we specify to join by users.id
-- and comments.user_id, so these columns will have to match.
...
-- Type of joins
-- 1. INNER JOIN -- default when we use join, we use (inner join).

-- 2. LEFT JOIN  -- If there are no values in the left table to link with the right table,
-- we still merge them, but we do not delete values without a link in the left table.

-- 3. RIGHT JOIN -- The same way the right join works only for the right table. if the right table is not linked
-- to the left table, the row is still added with null values in the left table.

-- 4. FULL JOIN  -- works like a left and right join if the left or right table has no data for links then just fills in
-- the empty null cells so we can fully merge the two tables.
...
-- Three way joins
-- We can many join. we merge the tables in order.
SELECT *
FROM photos

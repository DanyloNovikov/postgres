...
-- Joins ==> Use a join most times that you are asked to find data that involves multiple resources.

-- Aggregation ==> Looks at many rows and calculates a single value.
-- Aggregation ==> Words like 'most', 'average', 'least' are a sign that you need to use an aggregation.

-- Example question for use Join
...
-- Джоин присоединяет таблицу users и так как в таблице comments указаны связи с таблицей user мы указываем присоединить
-- по users.id и comments.user_id тобишь эти колонки должны будут совпадать

select url, contents from comments join photos on photos.id = comments.photos_id;

-- Notes on joins
-- Table order between 'FROM' and 'JOIN' frequently makes a different
-- We must give context in column names collide
...
-- we can not see photos if user_id null because we must to have relative with both tables.

-- type of joins
-- 1. inner join -- default when we use join, we can also use (inner join).
-- 2. left join  -- если в левой таблице нет значений для связи с правой таблицей мы их всеравно обьединяем но
-- не удаляем значений без связи в левой таблице.
-- 3. right join -- аналогичным образом работает right join только для правой таблицы. если вправой таблице нет связи с
-- c левой таблицей строка всеравно добавляеться с null значениями влевой таблице.
-- 4. full join  -- работает как left и right join если влевой или правой таблице нет данных для связей тогда
-- просто заполняет пустые ячейки null таким образом мы можем полность обьеденить 2 таблицы
...
select *
from photos
join comments on photos.id = comments.photos_id
left join users on photos.user_id = users.id and comments.user_id =  users.id;
